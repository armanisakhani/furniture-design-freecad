...
- `orders/` — combines 1+ furniture items from `furniture/<name>/` into one named order: builds each item (optionally with its own style/layout/color/etc. override), lays them out side by side in one `.FCStd`, and reports one combined cut list across all of them. A named order is one readable YAML file, `orders/specs/<name>.yaml` (see `orders/registry.py`'s `load_order` for the schema); `tools/order.sh <name>` builds it and opens it in FreeCAD, `tools/order.sh <name> --report` builds it and opens its full HTML cutlist report, and `make order NAME=<name>` does the same as the plain view command.
